...
import { formatCityToLabelValue } from '../../../containers/Providers/add/utils';
const styles = theme => ({
  root: {
...
export function AddProviderFrom(props) {
  const { cities, editMode, classes, errors, formData, handleFormDataChange, handleSubmit, handleAnuler } = props;

  //   const formattedCities = cities.map(formatCityToLabelValue)
  // console.log("form" ,cities);
  return (
    <Paper className={classes.paper}>
      <Grid className={classes.headerGridContainer} container>
        <Grid xs={12} className={classes.title} item>
          <Typography variant="h5" color="primary">
            {`Informations des Grossistes`}
          </Typography>
        </Grid>
      </Grid>
...
AddProviderFrom.defaultProps = {};

AddProviderFrom.propTypes = {
...
export default withStyles(styles)(AddProviderFrom);
