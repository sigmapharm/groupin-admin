...
import { adminCommandArticlesHeadersForUpdate, articleHeaders, articleHeadersForUpdate } from '../../../headers';
...
export default withStyles(styles)(({ list, metadata, classes, readMode = true, onChange, isAdmin, forAdmin, copyQuantities }) => {
  const deliveredTotalAmount =
    _.sumBy(
      list,
      ({ modifiedQuantity, computedPPH, selected, tva }) =>
        selected * (modifiedQuantity || 0) * (computedPPH || 0) + (modifiedQuantity || 0) * (computedPPH || 0) * (tva / 100),
    ) || 0;
  ...
  return (
    <>
      <div className={classes.metaContainer}>
        ...
        <div className={classes.metaItems}>
          <Typography color="textSecondary">Total gain</Typography>
          <Typography variant="h6" component="h2">
            {(metadata.totalAmount - metadata.totalAmountDiscount || 1).toFixed(2)}
          </Typography>
        </div>
        {/* <div className={classes.metaItems}>
            ...
            : isAdmin
              ? forAdmin
                ? adminCommandArticlesHeadersForUpdate(
                    <>
                      <span>QUANTITE</span>
                      <Tooltip placement="top" title="Copier les quantités vers les quantités livrées">
                        <IconButton onClick={copyQuantities} style={{ padding: 5 }}>
